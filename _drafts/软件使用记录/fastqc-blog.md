...
- -o：指定输出文件夹，需要事先自行创建
- -t：线程数。一个线程使用 250M 内存；32位系统不要超过6个线程。
- --extract：默认参数。输出文件为解压后文件
- --noextract：设置输出文件不解压
- -h：帮助

### 总结fastqc 文件

写了个脚本总结 fastqc 结果文件，暂时仅提取了reads 数，reads长度，%GC，Q20，Q30
其它数据，有需要再提取
```
#!/bin/python3
...
#get all _fastqc.zip in current path
